...
public:
    bool is_on_path(const Path& path, double x, double y, double threshold = 0.2) {
        int idx = find_closest_index_internal(path, x, y);
        return std::hypot(path.X[idx] - x, path.Y[idx] - y) < threshold;
    }

    bool check_hv_collision(const Path& my_path, int my_idx, const std::map<int, Point>& others_pos,
                            const std::map<int, Path>& others_paths, const std::map<int, bool>& others_received) {
        int my_path_size = static_cast<int>(my_path.X.size());
        double dist_sq_threshold = 0.5 * 0.5;

        for (int id : {19, 20}) {
            if (others_received.find(id) == others_received.end() || !others_received.at(id)) continue;
            const auto& hv_path = others_paths.at(id);
            int hv_path_size = static_cast<int>(hv_path.X.size());
            int hv_idx = find_closest_index_internal(hv_path, others_pos.at(id).x, others_pos.at(id).y);

            double my_accum = 0.0;
            for (int i = my_idx; i < my_path_size - 1 && my_accum < 0.5; ++i) {
                double dx_my = my_path.X[i+1] - my_path.X[i];
                double dy_my = my_path.Y[i+1] - my_path.Y[i];
                my_accum += std::sqrt(dx_my*dx_my + dy_my*dy_my);

                double hv_accum = 0.0;
                for (int j = hv_idx; j < hv_path_size - 1 && hv_accum < 2.3; ++j) {
                    ...
                    if (dx_cross*dx_cross + dy_cross*dy_cross < dist_sq_threshold) {
                        if (hv_accum < my_accum + 0.1) return true;
                    }
                }
            }
        }
    ...
    bool check_hardcoded(const Path& my_path, int my_idx, int other_id, const Path& other_path,
                         const std::map<int, Point>& others_pos, const std::map<int, bool>& others_received, int my_logical_id) {
        if (others_received.find(other_id) == others_received.end() || !others_received.at(other_id)) return false;

        if (!is_on_path(other_path, others_pos.at(other_id).x, others_pos.at(other_id).y, 0.2)) {
            return false;
        }

        int other_idx = find_closest_index_internal(other_path, others_pos.at(other_id).x, others_pos.at(other_id).y);
        int my_path_size = static_cast<int>(my_path.X.size());
        int other_path_size = static_cast<int>(other_path.X.size());

        double my_accum = 0.0;
        double dist_sq_threshold = 0.25 * 0.25; // 의미:
        ...
        for (int i = my_idx; i < my_path_size - 1 && my_accum < 1.0; ++i) {
            ...
            for (int j = other_idx; j < other_path_size - 1 && other_accum < 1.5; ++j) {
                ...
                if (dx_cross*dx_cross + dy_cross*dy_cross < dist_sq_threshold) {
                    ...
                    if (std::abs(my_accum - other_accum) <= 0.15 && my_logical_id > other_id) return true;
                }
            }
        }
        return false;
